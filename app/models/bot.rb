class Bot < ActiveRecord::Base
  ...
  def self.ocs_mentions
    CLIENT.search("#orlandocitysoccer", result_type: "recent").take(5).each do |t|
  ...
  def self.orlando_soccer
    CLIENT.search("Orlando soccer", result_type: "recent").take(5).each do |t|
      User.create(name: t.user.screen_name, tweet_id: t.id.to_s)
      CLIENT.retweet(t)
    end
  end
  ...
  def self.make_it_count
    CLIENT.search("Orlando #makeItCount", result_type: "recent").take(5).each do |t|
      User.create(name: t.user.screen_name, tweet_id: t.id.to_s)
      CLIENT.retweet(t)
    end
  end
